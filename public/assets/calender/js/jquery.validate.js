 ...
 *
 * $Id: jquery.validate.js 5902 2008-10-21 18:44:07Z joern.zaefferer $
 *
	...
	methods: {
		...
		required: function(value, element, param) {
			...
			case 'select':
				...
				return options.length > 0 && ( element.type == "select-multiple" || parseInt(options[0].value) > 0);
			case 'input':
				if ( this.checkable(element) )
					return this.getLength(value, element) > 0;
			default:
				return $.trim(value).length > 0 && parseInt($.trim(value))>0; // original code : return $.trim(value).length > 0. && $.trim(value)>0 is added by mizan for checking 0
			}
		},
		...
		number: function(value, element) {
			return this.optional(element) || /^[1-9](?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(value);
			//return this.optional(element) || /^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(value); //original code
		},
